...
export function normalizeMulti(options={}) {
  if (!Array.isArray(options)) {
    ...
    let pins = options.pins.map(pin => {
      return normalizeIO(pin);
    });
    
    // If IO is defined on options use it as default
    if (options.io) {
      pins.forEach(pin => pin.io = pin.io || options.io);
      delete options.io;
    }
    
    // Copy each property that is not ```pins``` onto each member of the pins array
    delete options.pins;
    pins.map(pin => { return Object.assign(pin, options) });
    options = pins;
    return options;
  } else {
    return options.map(pin => {
      return normalizeIO(pin)
    });
  }
